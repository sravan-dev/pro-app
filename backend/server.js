...
app.get('/api/students', async (req, res) => {
  ...
  res.json(await db.all("SELECT u.id,u.name,u.email,u.role,u.specialization,u.status,u.avatar_color,u.created_at,u.team_id, (SELECT name FROM teams WHERE id=u.team_id) AS team_name, COUNT(e.enrollment_id) as enrolled_courses, ROUND(AVG(e.progress_percentage),1) as avg_progress FROM users u LEFT JOIN enrollments e ON e.student_id=u.id WHERE u.role='student' GROUP BY u.id ORDER BY u.name"));
});
